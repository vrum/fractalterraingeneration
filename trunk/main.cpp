...
int main(int argc, char* argv[])
{
	...
	int iterations = 4;
	string erosionAlg = "";
	float talus = 0.4f / 500.0f;
	float evaporation = 0.9f;
	float phi = 0.8f;
	...
	for (i = 1; i < (unsigned)argc; ++i)
	{
		...
		if (pos != string::npos)
		{
			...
			else if (pre == "erosion" || pre == "eAlgorithm" || pre == "erosionAlgorithm")
			{
				erosionAlg = post;
			}
			else if (pre == "eTalus" || pre == "talus" || pre == "erosionTalus")
			{
				talus = atof(post.c_str());
			}
			else if (pre == "erosionIterations" || pre == "iterations")
			{
				iterations = atoi(post.c_str());
			}
			else if (pre == "evaporation")
			{
				evaporation = atof(post.c_str());
			}
			else if (pre == "phi")
			{
				phi = atof(post.c_str());
	...
	//make the Erosion object
	Erosion* eroder = NULL;
	if (erosionAlg == "thermal" || erosionAlg == "Thermal")
		eroder = new Thermal_erosion(talus);
	else if (erosionAlg == "inverse" || erosionAlg == "InverseThermal")
		eroder = new Inverse_thermal_erosion(talus);
	else if (erosionAlg == "hydraulic" || erosionAlg == "Hydraulic")
		eroder = new Hydraulic_erosion(0.1f, 0.1f, evaporation, 0.1f);
